...
# Determine the frontend build directory - trying multiple approaches for reliability
try:
    # First approach: relative path (as in original code)
    client = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "frontend", "dist")
    
    # If relative path doesn't exist, try current directory + frontend/dist
    if not os.path.exists(client):
        alt_client = os.path.join(os.getcwd(), "frontend", "dist")
        if os.path.exists(alt_client):
            client = alt_client
            logger.info(f"Using alternative frontend path: {client}")
    
    # If neither exists, try direct path (common in some deployments)
    if not os.path.exists(client):
        direct_client = os.path.join(os.getcwd(), "dist")
        if os.path.exists(direct_client):
            client = direct_client
            logger.info(f"Using direct frontend path: {client}")
    
    # Log frontend path for debugging
    logger.info(f"Frontend path: {client}")
    logger.info(f"Current directory: {os.getcwd()}")
    logger.info(f"Directory contents: {os.listdir(os.getcwd())}")
    
    if os.path.exists(client):
        logger.info(f"Frontend directory contents: {os.listdir(client)}")
    else:
        logger.warning(f"Frontend directory not found: {client}")
except Exception as e:
    logger.error(f"Error setting up frontend path: {str(e)}")
    # Default fallback
...
def index(filename):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error serving index.html: {str(e2)}")
            return jsonify({"error": "Frontend not found. Please check deployment configuration."}), 500
...
def debug_info():
    info = {
        'environment': {
            ...
            'frontend_exists': os.path.exists(client)
        }
    }
    
    if os.path.exists(client):
        info['frontend'] = {
            'contents': os.listdir(client)
        }
    
    return jsonify(info), 200

if __name__ == '__main__':
    # Get port from environment variable (required for Render) or default to 10000
